...
export const postLabelMacrosOnly = async (req: Request, res: Response) => {
    try {
        // ✅ hardcoded image for testing
        const image_url = String(req.body?.image_url ?? "").trim()

        const mimeType = guessMimeTypeFromUrl(image_url)

        const imgRes = await axios.get<ArrayBuffer>(image_url, {
            responseType: "arraybuffer",
            timeout: 15000,
            headers: { "User-Agent": "CarboTrackr/1.0" },
        ...
        const prompt = [
            ...
            "",
            "Rules:",
            "1) Do NOT estimate or guess. If a value is not clearly visible, output null for that field.",
            "2) Output must be valid JSON and match exactly this schema:",
            '{"calories_kcal": number|null, "carbs_g": number|null, "protein_g": number|null, "fat_g": number|null, "confidence": number}',
            "3) Use numbers only (no strings like '20g').",
        ...
        const parsedOut = responseSchema.safeParse(json)
        if (!parsedOut.success) {
            return res.status(502).json({
                error: "Gemini JSON did not match expected schema",
                details: parsedOut.error.flatten(),
                raw: json,
            })
        }
        ...
        return res.json({
            ok: true,
            macros_per_serving: {
                calories_kcal: out.calories_kcal,
                carbs_g: out.carbs_g,
        ...
        const details = e?.response?.data ?? e?.message ?? String(e)
        return res.status(status).json({ error: "Failed to parse label macros", details })
    }
}
