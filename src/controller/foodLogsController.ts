...
export async function fetchFatSecretServingDetails(
  ...
  const match = servingsArr.find(
    (s: any) => String(s?.serving_id ?? "") === serving_id,
  );
...
export const getFoodByQuery = async (req: Request, res: Response) => {
  try {
    ...
    const page_number = Number.isFinite(Number(req.query.page_number))
      ? String(Number(req.query.page_number))
      : "0";

    // ✅ increase FOOD results so branded items can appear
    const max_results = Number.isFinite(Number(req.query.max_results))
      ? String(Number(req.query.max_results))
      : "20";
    ...
    if (!oauth_consumer_key || !oauth_consumer_secret) {
      return res
        .status(500)
        .json({ error: "FatSecret OAUTH credentials are not configured" });
    }
...
export const getFoodDetailsByServingId = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    if (!serving_id)
      return res
        .status(400)
        .json({ error: "serving_id route param is required" });
...
export const postFoodLog = async (req: Request, res: Response) => {
  try {
    ...
    if (!food_name) {
      return res
        .status(500)
        .json({ error: "FatSecret response missing food_name" });
    }
