...
export const healthMetrics = createTable("health_metrics",
    ...
    (t) => [
        check("health_metrics_daily_calorie_goal_kcal_gt_0", sql`${t.daily_calorie_goal_kcal} > 0`),
        check("health_metrics_daily_carbohydrate_goal_g_gt_0", sql`${t.daily_carbohydrate_goal_g} > 0`),
        check("health_metrics_reminder_frequency_0_to_3", sql`${t.reminder_frequency} BETWEEN 0 AND 3`),
    ]
)
...
export const bloodGlucoseUnitsEnum = pgEnum("blood_glucose_units", ["MG_DL", "MMOL_L"])
export const bloodGlucoseMeasurements = createTable("blood_glucose_measurements",
    ...
    (t) => [
        check("blood_glucose_level_gt_0", sql`${t.level} > 0`),
    ]
)


export const mealTypeEnum = pgEnum("meal_type", ["BREAKFAST", "LUNCH", "DINNER", "SNACK"])
export const foodSourceTypeEnum = pgEnum("food_source_type", ["FAT_SECRET_API", "AMAZON_SAGE_MAKER_API", "GOOGLE_GEMINI_API"])
...
export const topicTypeEnum = pgEnum("faq_main_topic", ["ACCOUNT", "HEALTH", "FOOD_LOG", "SCANNERS", "REPORTS"])
...
export const calorieData = createTable("calorie_data",
    ...
    (t) => [
        check("calorie_data_calorie_goal_kcal_gt_0", sql`${t.calorie_goal_kcal} > 0`),
        check("calorie_data_calorie_actual_kcal_gt_0", sql`${t.calorie_actual_kcal} > 0`),
    ]
)
...
export const carbohydrateData = createTable("carbohydrate_data",
    ...
    (t) => [
        check("carbohydrate_data_carbohydrate_goal_g_gt_0", sql`${t.carbohydrate_goal_g} > 0`),
